...
def extract_proof_status(report_content: str) -> str:
    """
    Extract the proof status from the JasperGold or SymbiYosys report.

    Args:
        report_content (str): Content of the JasperGold/SBY report.

    Returns:
        str: The proof status (proven, cex, inconclusive, or error).
    """
    # SymbiYosys Check
    if "DONE (PASS, rc=0)" in report_content:
        return "proven"
    if "DONE (FAIL" in report_content:
        return "cex"
    if "SBY" in report_content and "ERROR:" in report_content:
        return "error"
